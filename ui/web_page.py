...
def launch_page(today, live_df, upcoming_df, finished_df, scoreboard_raw_df):
    ...
    with st.expander("Select categories for game ratings",expanded=True):
        ...
        with col1:
            cat1 = st.slider(label = "Point Differential", min_value = 0, max_value = 10, value = 10, step = 1)
        with col2:
            cat2 = st.slider(label = "Time Remaining", min_value = 0, max_value = 10, value = 10, step = 1)
        with col3:
            cat3 = st.slider(label = "Player Availability", min_value = 0, max_value = 10, value = 9, step = 1)
        with col4:
            cat4 = st.slider(label = "Team Quality", min_value = 0, max_value = 10, value = 9, step = 1)
        with col5:
            cat5 = st.slider(label = "Style Contrasts", min_value = 0, max_value = 10, value = 8, step = 1)
        with col6:
            cat6 = st.slider(label = "Zach Lowe Rankings", min_value = 0, max_value = 10, value = 8, step = 1)
        with col7:
            cat7 = st.slider(label = "Offensive Rating", min_value = 0, max_value = 10, value = 7, step = 1)
        
        col8, col9, col10, col11, col12, col13, col14 = st.columns(7)
        with col8:
            cat8 = st.slider(label = "Game Flow", min_value = 0, max_value = 10, value = 7, step = 1)
        with col9:
            cat9 = st.slider(label = "Foul Rate", min_value = 0, max_value = 10, value = 7, step = 1)
        with col10:
            cat10 = st.slider(label = "Assist Percent", min_value = 0, max_value = 10, value = 6, step = 1)
        with col11:
            cat11 = st.slider(label = "Egalitarian Offense", min_value = 0, max_value = 10, value = 6, step = 1)
        with col12:
            cat12 = st.slider(label = "Heroic Performance", min_value = 0, max_value = 10, value = 6, step = 1)
        with col13:
            cat13 = st.slider(label = "Pace", min_value = 0, max_value = 10, value = 6, step = 1)
        with col14:
            cat14 = st.slider(label = "National Broadcast", min_value = 0, max_value = 10, value = 6, step = 1)
